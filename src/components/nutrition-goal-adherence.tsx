    ...
    <section className="grid gap-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => {
          ...
          return (
            <article className="card p-5" key={metric.key}>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-muted">
              Блок показывает, насколько ты придерживаешься дневных целей по КБЖУ.
              Средний процент считается по настроенным целям и не завышается выше 100%.
            </p>
          </div>

          <div className="grid gap-2 rounded-3xl border border-border bg-white/60 px-5 py-4 text-sm text-muted">
            <p>
              Среднее выполнение:{" "}
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-[1.4fr_0.6fr]">
            ...
            {trend.map((day) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 flex min-h-36 items-end">
                    <div className="w-full rounded-3xl bg-white/70 p-2">
                      <div
                        className="rounded-2xl bg-accent/85 transition-[height] duration-300"
                        style={{ height: `${Math.max(score, 6)}px` }}
                      />
                    </div>
                  </div>
                  <p className="mt-3 text-lg font-semibold text-foreground">{score}%</p>
          ...
          <div className="grid gap-3">
            {metrics.map((metric) => {
              ...
              return (
                <article className="rounded-3xl border border-border bg-white/60 p-4" key={metric.key}>
                  <p className="text-sm text-muted">{metric.label}</p>
                  <p className="mt-2 text-2xl font-semibold text-foreground">
