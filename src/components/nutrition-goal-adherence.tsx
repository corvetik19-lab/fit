    ...
    <section className="grid gap-4">
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        {metrics.map((metric) => {
          ...
          return (
            <article className="surface-panel surface-panel--soft p-3" key={metric.key}>
              ...
              <div className="mt-2.5 h-1.5 rounded-full bg-white/70">
                <div
      ...
      <section className="surface-panel p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="workspace-kicker">Тренд</p>
            <h2 className="mt-2 text-lg font-semibold text-foreground sm:text-xl">
              Питание за 7 дней
            </h2>
            <p className="mt-1.5 max-w-2xl text-sm leading-6 text-muted">
              Короткий weekly-view без длинных карточек: видно ритм, средний процент и активные дни.
            </p>
          </div>

          <div className="grid min-w-[12rem] gap-2 rounded-[0.84rem] border border-border bg-white/84 px-3 py-2.5 text-sm text-muted">
            <p>
              Среднее:{" "}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-7">
            {trend.map((day) => {
              ...
              return (
                <article
                  className="rounded-[0.84rem] border border-border bg-white/82 p-3"
                  key={day.summary_date}
                >
                  <p className="text-xs uppercase tracking-[0.16em] text-muted">
                    {getDayLabel(day.summary_date)}
                  </p>
                  <div className="mt-3 flex min-h-18 items-end">
                    <div className="w-full rounded-[0.78rem] bg-[color-mix(in_srgb,var(--surface-strong)_82%,white)] p-2">
                      <div
                        className="rounded-[0.72rem] bg-accent/85 transition-[height] duration-300"
                        style={{ height: `${Math.max(Math.round(score * 0.56), 8)}px` }}
                      />
                    </div>
                  </div>
                  <p className="mt-2 text-base font-semibold text-foreground">{score}%</p>
          ...
          {metrics.map((metric) => {
            ...
            const average = target
              ? Math.round(
                  trend.reduce((sum, day) => {
                    return (
                      sum + Math.min((Number(day[metric.key]) / target) * 100, 100)
                    );
                  }, 0) / trend.length,
                )
              : null;
            ...
            return (
              ...
              >
                ...
                <p className="mt-1 text-[0.72rem] text-muted">
                  {target ? "Среднее по цели" : "Нет цели"}
                </p>
              </article>
            );
          })}
