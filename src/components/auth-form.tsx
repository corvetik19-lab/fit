  ...
  function submit(event?: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        setError(
          mode === "sign-up"
            ? "Сервис регистрации временно недоступен. Попробуй еще раз немного позже."
            : "Сервис входа временно недоступен. Попробуй еще раз немного позже.",
        );
      } finally {
        setIsPending(false);
      }
      ...
      <section className="w-full max-w-[21rem]">
        <div className="grid gap-3.5">
              ...
              return (
                <button
                  className={`flex-1 rounded-[0.8rem] px-3 py-2 text-sm font-semibold transition ${
                    active
                      ? "bg-[linear-gradient(135deg,#2563EB,#0891FF,#2DD4BF)] text-white shadow-[0_10px_18px_-18px_rgba(8,145,255,0.45)]"
          ...
          <form className="grid gap-3" onSubmit={submit}>
            ...
            <label className="grid gap-2">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="example@fitora.app"
                  required
                  spellCheck={false}
                  type="email"
                  value={email}
            ...
            <button
              className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-[0.82rem] border border-[#2DD4BF]/18 bg-[linear-gradient(135deg,#2563EB,#0891FF,#2DD4BF)] px-4 py-2.5 text-sm font-bold text-white shadow-[0_12px_20px_-18px_rgba(8,145,255,0.45)] transition hover:translate-y-[-1px] disabled:cursor-not-allowed disabled:opacity-60"
              disabled={
                isPending ||
                !email.trim() ||
                !password.trim() ||
