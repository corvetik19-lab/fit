...
export function createFallbackAdminUsersResponse(input: {
...
export function parseRoleFilter(value: string | null): AdminRoleFilter {
  switch (value) {
    case "super_admin":
    case "support_admin":
    case "analyst":
...
export function parseSortKey(value: string | null): AdminUsersSortKey {
  switch (value) {
    case "activity_desc":
    case "sign_in_desc":
    case "workout_desc":
...
async function listAllAuthUsers(adminSupabase: AdminUsersSupabase) {
  const users: User[] = [];
  let page = 1;
...
export async function loadAdminUsersData(params: {
  activityFilter: ActivityFilter;
  adminSupabase: AdminUsersSupabase;
  query: string;
  roleFilter: AdminRoleFilter;
  sortKey: AdminUsersSortKey;
}) {
  const { activityFilter, adminSupabase, query, roleFilter, sortKey } = params;
  const [
