...
alter table public.workout_sets
  ...
  drop column if exists set_note;
...
begin
  ...
  if tg_table_name = 'workout_days' then
    if (to_jsonb(new) - 'status' - 'body_weight_kg' - 'session_note' - 'session_duration_seconds' - 'updated_at') <>
       (to_jsonb(old) - 'status' - 'body_weight_kg' - 'session_note' - 'session_duration_seconds' - 'updated_at') then
      raise exception 'Locked workout days only allow status/body_weight_kg/session_note/session_duration_seconds updates.';
    end if;
  elsif tg_table_name = 'workout_exercises' then
