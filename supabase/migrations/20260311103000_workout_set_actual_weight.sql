...
alter table public.workout_sets
  drop constraint if exists workout_sets_actual_weight_kg_check;

alter table public.workout_sets
  add constraint workout_sets_actual_weight_kg_check
...
begin
  ...
  if tg_table_name = 'workout_days' then
    ...
    if (to_jsonb(new) - 'actual_reps' - 'actual_weight_kg' - 'updated_at') <>
       (to_jsonb(old) - 'actual_reps' - 'actual_weight_kg' - 'updated_at') then
      raise exception 'Locked workout sets only allow actual_reps/actual_weight_kg updates.';
    end if;
  end if;
