...
### User content imagery
  ...
  - [x] Р”РѕР±Р°РІРёС‚СЊ owner-scoped `image_url` РґР»СЏ РїРѕР»СЊР·РѕРІР°С‚РµР»СЊСЃРєРёС… СѓРїСЂР°Р¶РЅРµРЅРёР№ Рё РїСЂРѕРґСѓРєС‚РѕРІ СЃ preview РІ РїСЂРёР»РѕР¶РµРЅРёРё Рё РІР°Р»РёРґР°С†РёРµР№ Р°Р±СЃРѕР»СЋС‚РЅС‹С… URL РЅР° API-СѓСЂРѕРІРЅРµ.
  - [x] Р”РѕР±Р°РІРёС‚СЊ РѕС‚РґРµР»СЊРЅС‹Р№ super-admin РєРѕРЅС‚СѓСЂ РґР»СЏ РїСЂР°РІРєРё РёР·РѕР±СЂР°Р¶РµРЅРёР№ СѓРїСЂР°Р¶РЅРµРЅРёР№ Рё РїСЂРѕРґСѓРєС‚РѕРІ РёР· РєР°СЂС‚РѕС‡РєРё РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ СЃ Р°СѓРґРёС‚РѕРј РґРµР№СЃС‚РІРёР№.
...
## 2026-04-14 Supabase runtime и nutrition camera/import follow-up

- [x] Supabase runtime для `fit` снова подтверждён как рабочий на проекте `nactzaxrjzsdkyfqwecf`: `npm run verify:supabase-runtime` проходит зелёно и больше не упирается в старый DNS-блокер.
- [x] Mobile-first camera/import контур в [nutrition-photo-analysis.tsx](/C:/fit/src/components/nutrition-photo-analysis.tsx) доведён до рабочего состояния: загрузка фото по реальному user gesture, превью, AI-анализ и сохранение через [photo-import route](/C:/fit/src/app/api/nutrition/photo-import/route.ts) теперь подтверждены regression-тестом.
- [x] Для nutrition regression устранён post-restore редирект в онбординг: [nutrition-capture.spec.ts](/C:/fit/tests/e2e/nutrition-capture.spec.ts) теперь завершает анкету при необходимости и после этого проверяет barcode/import и photo-import flow на чистом рабочем маршруте.
- [x] Для AI history regression устранён Supabase seeding-флак: [tests/e2e/helpers/ai.ts](/C:/fit/tests/e2e/helpers/ai.ts) получил усиленный retry и батчевый `replaceAiChatHistory`, а [ai-workspace.spec.ts](/C:/fit/tests/e2e/ai-workspace.spec.ts) больше не зависит от серии мелких insert-операций.
- [x] Auth-based regression адаптирован к живому сценарию после восстановления базы: [authenticated-app.spec.ts](/C:/fit/tests/e2e/authenticated-app.spec.ts) и [helpers/auth.ts](/C:/fit/tests/e2e/helpers/auth.ts) теперь корректно проходят путь `dashboard -> onboarding -> dashboard`, если тестовый пользователь ещё не завершил профиль.
...
## 2026-04-14 agent autonomy, governance и self-evolving lane

- [x] Создан отдельный execution-doc [CODEX_AGENT_AUTONOMY_PLAN.md](/C:/fit/docs/CODEX_AGENT_AUTONOMY_PLAN.md), а рядом добавлены [CODEX_AGENT_GOVERNANCE.md](/C:/fit/docs/CODEX_AGENT_GOVERNANCE.md) и generated-doc [CODEX_AGENT_REGISTRY.md](/C:/fit/docs/CODEX_AGENT_REGISTRY.md), чтобы orchestration, allowlist, kill switch и autonomous mainline lane были оформлены как отдельный управляемый слой.
- [x] В [`.codex/config.toml`](/C:/fit/.codex/config.toml) добавлены роли `orchestrator`, `autonomy_guardian`, `evolution_driver`, а в `.agents/skills/` — навыки `fit-agent-orchestration`, `fit-agent-governance`, `fit-agent-evolution`.
- [x] Agent layer получил исполняемые scripts [agent-governance-config.mjs](/C:/fit/scripts/agent-governance-config.mjs), [agent-inventory.mjs](/C:/fit/scripts/agent-inventory.mjs), [sync-codex-agent-registry.mjs](/C:/fit/scripts/sync-codex-agent-registry.mjs), [verify-agent-governance.mjs](/C:/fit/scripts/verify-agent-governance.mjs), [agent-evolve.mjs](/C:/fit/scripts/agent-evolve.mjs) и команды `agent:*` плюс `verify:agent-governance`.
- [x] CI и automation слой синхронизирован: [quality.yml](/C:/fit/.github/workflows/quality.yml) прогоняет `verify:agent-governance`, а новый workflow [agent-autonomy.yml](/C:/fit/.github/workflows/agent-autonomy.yml) даёт scheduled/manual governance sweep и optional direct-to-main write lane через GitHub variable `CODEX_AGENT_AUTONOMY_ENABLED`.
- [x] Developer docs синхронизированы по tranche: обновлены [AGENTS.md](/C:/fit/AGENTS.md), [code_review.md](/C:/fit/code_review.md), [README.md](/C:/fit/README.md), [docs/README.md](/C:/fit/docs/README.md), [CODEX_PLAYBOOK.md](/C:/fit/docs/CODEX_PLAYBOOK.md), [CODEX_ONBOARDING.md](/C:/fit/docs/CODEX_ONBOARDING.md), [CODEX_ROLLOUT_PLAN.md](/C:/fit/docs/CODEX_ROLLOUT_PLAN.md) и [CODEX_AGENT_HARDENING_PLAN.md](/C:/fit/docs/CODEX_AGENT_HARDENING_PLAN.md).
- [x] Tranche подтверждён пакетами `npm run agent:sync-registry`, `npm run agent:evaluate`, `npm run verify:codex`, `npm run verify:agent-governance`, `npm run lint`, `npm run typecheck`, `npm run build`.
- [x] Общий progress execution checklist не меняется и остаётся `199 / 204` (`98%`), потому что это ещё один developer-facing hardening slice поверх уже существующего продукта, а внешними блокерами по-прежнему остаются live `CloudPayments`, production `Sentry` env и полноценный embeddings runtime.

## 2026-04-14 deploy wait и плановый прогресс в agent contract

- [x] Agent contract усилен двумя обязательными operational правилами: deploy-oriented tranche не считаются завершёнными до clean Vercel deployment, а каждый существенный статус и финальный handoff должны показывать текущий прогресс `MASTER_PLAN` в формате `done / total (percent%)`.
...
- [x] `src/lib/viewer.ts` переведён на slice-by-slice snapshot loading с частичными fallback-ответами: деградация `onboarding/admin-state` больше не валит целиком `/dashboard`, `/workouts`, `/nutrition`, `/history`, `/settings` и `/ai`, если Supabase отвечает нестабильно.
- [x] `src/app/api/ai/chat/route.ts`, `src/app/api/ai/assistant/route.ts`, `src/app/api/ai/meal-plan/route.ts` и `src/app/api/ai/workout-plan/route.ts` усилены transient retry и timeout-контуром; chat теперь отдаёт детерминированный резервный ответ, а plan routes больше не держат лишний roundtrip на `select().single()` при создании proposal.
- [x] `src/lib/ai/proposals.ts` и `src/lib/ai/plan-generation.ts` уплотнены под быстрый persist: proposal id создаётся на сервере заранее, payload сохраняет только компактный `contextSnapshot` и короткий `knowledge` summary, а usage increment в plan routes стал best-effort и не удерживает HTTP-ответ.
- [x] `src/components/ai-chat-panel.tsx` и `src/components/ai-chat-panel-model.ts` убирают ложный runtime notice, если ассистент уже успел показать полноценный резервный ответ; для пользователя это больше не выглядит как падение AI при успешном fallback.
- [x] Функциональная проверка зелёная на свежем runtime `http://127.0.0.1:3119`: `npm run lint`, `npm run typecheck`, `npm run build`, `node scripts/run-playwright.mjs PLAYWRIGHT_BASE_URL=http://127.0.0.1:3119 -- test tests/ai-gate/ai-quality-gate.spec.ts --workers=1 --reporter=dot` -> `5 passed`, `node scripts/run-playwright.mjs PLAYWRIGHT_BASE_URL=http://127.0.0.1:3119 -- test tests/e2e/ai-workspace.spec.ts tests/e2e/nutrition-capture.spec.ts tests/e2e/workout-focus-flow.spec.ts tests/e2e/settings-billing.spec.ts --workers=1 --reporter=dot` -> `9 passed`.
- [x] После этого общий progress [MASTER_PLAN.md](/C:/fit/docs/MASTER_PLAN.md) остаётся `220 / 223` (`99%`): кодовый AI/chat/nutrition/workout контур доведён, а незакрытым хвостом остаются только внешние runtime/env пункты про `CloudPayments` и production `Sentry`.
