...
- `/ai` проектируется как компактный coaching workspace, а не как “витрина AI”.
- В фокусе всегда три слоя: диалог, действия по планам и управляемая история чатов.
- История должна быть отзывчивой и понятной на телефоне: удаление одной сессии и очистка всей истории проектируются как быстрые действия без долгого подвешенного состояния.

### Admin detail

- `/admin/users/[id]` остаётся operator dossier: профиль, активность, billing, контент и операции читаются как один рабочий экран.
- В макетах нужно показывать не только обычное состояние, но и `degraded` вариант, когда часть данных недоступна, а интерфейс всё равно остаётся рабочим.
- Visual language тот же, что и у user shell, но плотность выше, вторичных объяснений меньше, а действия заметнее.
...
## 2026-04-15 Статус внедрения

- Подплан `Dark Utility` закрыт в коде как `10 / 10`.
- Базовый regression-пакет подтверждён на живом приложении:
  - `authenticated-app.spec.ts`
  - `admin-app.spec.ts`
  - `mobile-pwa-regressions.spec.ts`
- Для дальнейших дизайн-правок важно сохранять два внедрённых контракта:
  - auth/workspace проверки теперь завязаны на реальные заголовки экранов и shell title, а не на скрытые mobile controls;
  - workout focus-mode regression завязан на `data-testid="workout-regular-mode-button"`, а не на текст кнопки, поэтому label можно менять без ломки suite, если поведение и `testid` сохранены.
