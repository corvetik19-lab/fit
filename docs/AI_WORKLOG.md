...
### Dark Utility redesign: nutrition tranche

- Перевёл nutrition-capture слой в единый compact dark utility стиль: [nutrition-tracker.tsx](/C:/fit/src/components/nutrition-tracker.tsx), [nutrition-photo-analysis.tsx](/C:/fit/src/components/nutrition-photo-analysis.tsx), [nutrition-open-food-facts-card.tsx](/C:/fit/src/components/nutrition-open-food-facts-card.tsx), [nutrition-barcode-scanner.tsx](/C:/fit/src/components/nutrition-barcode-scanner.tsx), [nutrition-recipes-manager.tsx](/C:/fit/src/components/nutrition-recipes-manager.tsx) и [nutrition-meal-templates-manager.tsx](/C:/fit/src/components/nutrition-meal-templates-manager.tsx) больше не тянут светлые legacy-surface в camera/barcode/import flows.
- Привёл preview, input и success/error notices к тем же тёмным surface-уровням, что и остальной mobile shell, чтобы `/nutrition` воспринимался как один цельный рабочий экран.
- Проверил tranche командами `npm run lint`, `npm run typecheck`, `npm run build`, `node scripts/run-playwright.mjs PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 -- test tests/e2e/nutrition-capture.spec.ts --workers=1` -> `3 passed`.

### Dark Utility redesign: AI и admin tranche

- Перевёл AI workspace в компактный coaching-формат через [ai-workspace.tsx](/C:/fit/src/components/ai-workspace.tsx), [ai-workspace-sidebar.tsx](/C:/fit/src/components/ai-workspace-sidebar.tsx), [ai-chat-composer.tsx](/C:/fit/src/components/ai-chat-composer.tsx), [ai-chat-notices.tsx](/C:/fit/src/components/ai-chat-notices.tsx), [ai-chat-panel-cards.tsx](/C:/fit/src/components/ai-chat-panel-cards.tsx), [ai-chat-transcript.tsx](/C:/fit/src/components/ai-chat-transcript.tsx), [ai-prompt-library.tsx](/C:/fit/src/components/ai-prompt-library.tsx) и [use-ai-chat-web-search.ts](/C:/fit/src/components/use-ai-chat-web-search.ts).
- Дожал историю AI-чата до устойчивого поведения: optimistic `delete/clear all`, client-side timeout и retry на Supabase-операциях в [ai-workspace.tsx](/C:/fit/src/components/ai-workspace.tsx) и [chat.ts](/C:/fit/src/lib/ai/chat.ts); таргетированный [ai-workspace.spec.ts](/C:/fit/tests/e2e/ai-workspace.spec.ts) теперь проходит полностью.
- Перевёл admin surfaces в тот же dark utility язык и добавил server-first initial payload/retry-fallback для detail-экрана через [admin-dashboard-workspace.tsx](/C:/fit/src/components/admin-dashboard-workspace.tsx), [admin-users-directory.tsx](/C:/fit/src/components/admin-users-directory.tsx), [admin-user-detail.tsx](/C:/fit/src/components/admin-user-detail.tsx), [admin-user-detail-state.ts](/C:/fit/src/components/admin-user-detail-state.ts), [admin/users/[id]/page.tsx](/C:/fit/src/app/admin/users/[id]/page.tsx), [admin-auth.ts](/C:/fit/src/lib/admin-auth.ts) и [admin user detail route](/C:/fit/src/app/api/admin/users/[id]/route.ts).
- Проверка slice: `npm run lint`, `npm run typecheck`, `npm run build`, `npm run test:smoke`, direct `npx playwright test tests/e2e/ai-workspace.spec.ts --workers=1` с `PLAYWRIGHT_SKIP_AUTH_SETUP=1` и `PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100` -> `2 passed`. Полный `tests/e2e/admin-app.spec.ts` по-прежнему нестабилен из-за внешнего `Supabase/Auth` runtime (`ECONNRESET`, `ConnectTimeout`, `502`), это отдельно зафиксировано как внешний blocker.

### Dark Utility redesign: финальный regression tranche

- Закрыл последний чекбокс [DARK_UTILITY_REDESIGN_EXECUTION.md](/C:/fit/docs/DARK_UTILITY_REDESIGN_EXECUTION.md): новый стиль подтверждён как единый рабочий пакет для user, admin и mobile shell.
- В [authenticated-app.spec.ts](/C:/fit/tests/e2e/authenticated-app.spec.ts) заменил хрупкие ожидания по скрытым mobile-триггерам на кросс-layout проверки по реальным заголовкам экранов, поэтому auth suite теперь одинаково валиден для desktop и mobile shell.
...
### 2026-04-15 AI runtime resilience и функциональная проверка

- Усилил AI runtime против частичных сбоев `Supabase` и медленных provider-path: [viewer.ts](/C:/fit/src/lib/viewer.ts) теперь собирает снимок по частям с `isSnapshotPartial`, а [assistant route](/C:/fit/src/app/api/ai/assistant/route.ts) и [chat route](/C:/fit/src/app/api/ai/chat/route.ts) используют retry/timeout budget и не рушат UI при деградации.
- Ускорил proposal-first генерацию для meal/workout plans: в [plan-generation.ts](/C:/fit/src/lib/ai/plan-generation.ts) сохраняется компактный `contextSnapshot` и краткая knowledge summary, а [proposals.ts](/C:/fit/src/lib/ai/proposals.ts) больше не делает лишний `select().single()` после insert.
- В [meal-plan route](/C:/fit/src/app/api/ai/meal-plan/route.ts) и [workout-plan route](/C:/fit/src/app/api/ai/workout-plan/route.ts) usage tracking переведён в best-effort режим с timeout, чтобы пользователю всегда возвращался proposal даже при медленном feature-usage слое.
- В [ai-chat-panel-model.ts](/C:/fit/src/components/ai-chat-panel-model.ts) и [ai-chat-panel.tsx](/C:/fit/src/components/ai-chat-panel.tsx) убран ложный runtime-banner: если fallback-ответ уже показан, UI не рисует поверх него ошибку сети.
- Проверка tranche зелёная: `npm run lint`, `npm run typecheck`, `npm run build`, `node scripts/run-playwright.mjs PLAYWRIGHT_BASE_URL=http://127.0.0.1:3119 -- test tests/ai-gate/ai-quality-gate.spec.ts --workers=1 --reporter=dot` -> `5 passed`, `node scripts/run-playwright.mjs PLAYWRIGHT_BASE_URL=http://127.0.0.1:3119 -- test tests/e2e/ai-workspace.spec.ts tests/e2e/nutrition-capture.spec.ts tests/e2e/workout-focus-flow.spec.ts tests/e2e/settings-billing.spec.ts --workers=1 --reporter=dot` -> `9 passed`.
